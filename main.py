...
if __name__ == '__main__':
    ...
    myRDD.map(lambda entry: ([x for x in set(re.sub('(\W|\d)', ' ', entry['title']).lower().split()) if (x not in most_common_words) == True], entry['score'])) \
        ...
        .saveAsTextFile("scores_by_word_title")

    myRDD.filter(lambda entry: entry['url'] is not None)\
        .filter(lambda entry: entry['url'].startswith('http'))\
        .map(lambda entry: (re.compile("^(?:https?:\/\/)?(?:www\.)?([^:\/\n?]+)").findall(entry['url'])[0], entry['score']))\
        .groupByKey()\
        .mapValues(lambda scores: [np.mean(list(scores)), np.var(list(scores)), np.max(list(scores)), np.min(list(scores))])\
        .sortBy(lambda result: result[1][0], False)\
        .saveAsTextFile("scores_by_domain")

    myRDD.map(lambda entry: (entry['author'], entry['score']))\
        .groupByKey()\
        .mapValues(lambda scores: [np.mean(list(scores)), np.var(list(scores)), np.max(list(scores)), np.min(list(scores))])\
        .sortBy(lambda result: result[1][0], False)\
        .saveAsTextFile("scores_by_author")

    myRDD.map(lambda entry: ((datetime.datetime.fromtimestamp(entry['time']).year, datetime.datetime.fromtimestamp(entry['time']).month), entry['score']))\
        .groupByKey()\
        .mapValues(lambda scores: [np.mean(list(scores)), np.var(list(scores)), np.max(list(scores)), np.min(list(scores))])\
        .sortBy(lambda result: result[1][0], False)\
        .saveAsTextFile("scores_by_year_month")

    myRDD.map(lambda entry: (datetime.datetime.fromtimestamp(entry['time']).year, entry['score']))\
        .groupByKey()\
        .mapValues(lambda scores: [np.mean(list(scores)), np.var(list(scores)), np.max(list(scores)), np.min(list(scores))])\
        .sortBy(lambda result: result[1][0], False)\
        .saveAsTextFile("scores_by_year")

    myRDD.map(lambda entry: (datetime.datetime.fromtimestamp(entry['time']).day, entry['score']))\
        .groupByKey()\
        .mapValues(lambda scores: [np.mean(list(scores)), np.var(list(scores)), np.max(list(scores)), np.min(list(scores))])\
        .sortBy(lambda result: result[1][0], False)\
        .saveAsTextFile("scores_by_day")

    myRDD.map(lambda entry: (datetime.datetime.fromtimestamp(entry['time']).hour, entry['score']))\
        .groupByKey()\
        .mapValues(lambda scores: [np.mean(list(scores)), np.var(list(scores)), np.max(list(scores)), np.min(list(scores))])\
        .sortBy(lambda result: result[1][0], False)\
        .saveAsTextFile("scores_by_hour")
